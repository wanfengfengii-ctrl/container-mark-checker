...
| 目录 | 技术栈 | 说明 |
| --- | --- | --- |
| `backend/` | FastAPI + Pydantic | 校验算法、`POST /api/verify`、`POST /api/corrections`、字段级 422 |
| `frontend/` | React + TypeScript + Vite | 分段录入页面，PASS / FAIL 展示与疑似抄错诊断 |
...
`FAIL` 后闸口人员可从该结果发起「定位疑似抄错」。系统只针对**前十位**
（实填校验位保持不变）寻找能让实填校验位成立、且改动最小的候选：

- 编辑只允许**原字段内替换**或字段内**相邻交换**（所有者代码内部、
  序列号内部；不跨字段）；
- `I/L`、`O/Q`、`C/G` 组内替换与 `0/6/8/9` 组内替换、相邻交换成本为 **1**，
  其他合法替换成本为 **3**；
- 总路径成本上限 **2**；同一候选取最低路径成本后去重；
- 仅返回**全局最低成本**且校验成立的全部候选，按完整箱号（11 位）
  字典序排列。

接口 `POST /api/corrections` 复用与 `/api/verify` 相同的分段请求模型
与校验函数：

| 情形 | 响应 |
| --- | --- |
| 任一字段非法 | **422**，`loc` 末位为字段名 |
| 原编号本身已通过校验 | **409** |
| 预算内有解 | **200** `{"minimum_cost": <最低成本>, "candidates": [...]}` |
| 预算内无解 | **200** `{"minimum_cost": null, "candidates": []}` |

前端仅在 `FAIL` 后展示诊断入口；编辑任一字段立即清除诊断状态；
加载中、空结果与服务失败都会整体替换旧候选；点选候选后回填四段、
清空旧结论，再次提交仍按既有 PASS / FAIL 规则展示。
...
```bash
# 后端：映射、边界、余数 10->0、422 字段定位、校正有界搜索、接口联调
...
校正功能固定向量（实填编号 → `POST /api/corrections`）：

| 实填编号 | 最低成本 | 候选 |
| --- | --- | --- |
| `KEMZ0586318` | 1 | `KEMZ0506318`（唯一：单次 0/8 易混替换） |
| `CSQU5711716` | 2 | `CSOU5711176`、`SCQU5171716`（两步最低成本并列） |
| `CSQU5123116` | — | 空列表（预算内无解） |
